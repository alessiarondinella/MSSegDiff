...
class TwoConv(nn.Sequential):
    ...
    def __init__(
        ...
        if dim is not None:
            spatial_dims = dim
        conv_0 = Convolution(spatial_dims, in_chns, out_chns, act=act, norm=norm, dropout=dropout, bias=bias, padding=1) #3 5 64
        conv_1 = Convolution(
            spatial_dims, out_chns, out_chns, act=act, norm=norm, dropout=dropout, bias=bias, padding=1 #3 64 64
        )
...
class BasicUNetDe(nn.Module):
    ...
    def __init__(
        ...
        self.conv_0 = TwoConv(spatial_dims, in_channels, features[0], act, norm, bias, dropout) #3 5 64

        self.down_1 = Down(spatial_dims, fea[0], fea[1], act, norm, bias, dropout) #3 64 64
        ...
        self.final_conv = Conv["conv", spatial_dims](fea[5], out_channels, kernel_size=1) # 64 1
